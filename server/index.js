...
const installMode = fs.existsSync(path.join(__dirname, "..", ".git"))
  ? "git"
  : "npm";
...
const PROVIDER_WATCH_PATHS = [
  ...
  {
    provider: "gemini",
    rootPath: path.join(os.homedir(), ".gemini", "projects"),
  },
  {
    provider: "gemini_sessions",
  ...
  for (const { provider, rootPath } of PROVIDER_WATCH_PATHS) {
    try {
      ...
      watcher
        ...
        .on("unlink", (filePath) =>
          debouncedUpdate("unlink", filePath, provider, rootPath),
        )
        .on("addDir", (dirPath) =>
          debouncedUpdate("addDir", dirPath, provider, rootPath),
        )
        .on("unlinkDir", (dirPath) =>
          debouncedUpdate("unlinkDir", dirPath, provider, rootPath),
        )
        .on("error", (error) => {
    ...
    try {
      ...
      if (data.type === "claude-command") {
        ...
        const codexProjectPath =
          data.options?.projectPath || data.options?.cwd || null;
        ...
        const notificationHooks = codexUserId
          ? {
              onTurnCompleted: async ({
                ...
                if (
                  isSessionActivelyViewedByUser(codexUserId, "codex", sessionId)
                ) {
                  return;
                }
          ...
          shellProcess.onData((data) => {
            ...
            if (session.ws && session.ws.readyState === WebSocket.OPEN) {
              ...
              if (
                ...
                const bestUrl = dedupedDetectedUrls.reduce(
                  (longest, current) =>
...
async function startServer() {
  try {
    ...
    try {
      await primeCodexTitleMap();
    } catch (titleSyncError) {
      console.warn(
        "[WARN] Codex title map priming failed:",
        titleSyncError.message,
      );
    }
    ...
    server.listen(PORT, HOST, async () => {
      ...
      console.log("");
      console.log(
        `${c.info("[INFO]")} Server URL:  ${c.bright("http://" + DISPLAY_HOST + ":" + PORT)}`,
      );
